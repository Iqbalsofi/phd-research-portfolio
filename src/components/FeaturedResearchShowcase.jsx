...
import { BookOpen, Layers, CheckCircle2, AlertTriangle, Cpu, ArrowRight, ShieldCheck, FileText, Bug, Database, Info, Activity } from 'lucide-react';
import { mainProjectDetails, myContributions, demonstratedSkills } from '../data/researchData';

export default function FeaturedResearchShowcase() {
  const [activeTab, setActiveTab] = useState('rq3'); // Default to RQ3 centerpiece

  const { baselineFindings, researchQuestions, robustnessData, techSpecs, attribution } = mainProjectDetails;
      ...
      <div className="container">
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          <div className="subtle-badge" style={{ marginBottom: '0.75rem' }}>
            <BookOpen size={14} /> Primary Research Investigation
          </div>
          <h2 style={{ fontSize: '2.3rem', fontWeight: 800, lineHeight: 1.25 }}>
            Extending Semantic Communication from <span className="gradient-text">1D Signals to 3D Point Clouds</span>
          </h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', marginTop: '0.5rem', maxWidth: '820px' }}>
            An empirical investigation of the 3D <em>Discernment</em> framework (arXiv:2602.13556), discrete diffusion (DoT & SEDD), shape decoding, and channel corruption robustness on ModelNet40.
          </p>
        </div>

        {/* 1. Research Attribution & Role Clarity Box */}
        <div className="glass-card" style={{ 
          ...
          <div style={{ fontSize: '0.925rem', color: '#cbd5e1', lineHeight: 1.6, display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <p>
        ...
        {/* 2. Visual Architecture Pipeline Diagram */}
        <div className="glass-card" style={{ marginBottom: '3rem', padding: '1.75rem' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Layers size={18} color="var(--accent-cyan)" /> 3D Semantic Communication Pipeline Architecture
          </h3>
          ...
          <div style={{ 
            ...
            {[
              { label: '3D Point Cloud', sub: 'ModelNet40', icon: '🧊' },
              { label: 'MaskedVQVAE3D', sub: 'Encode + Quantize', icon: '📦' },
              { label: '4096 Tokens', sub: 'Sequence L=4096', icon: '🔢' },
              { label: 'Channel Loss', sub: 'Truncation / Puncturing', icon: '⚡' },
              { label: 'SEDD / DoT', sub: 'Reconstruction', icon: '🧠' },
              { label: 'MaskedVQVAE3D', sub: 'Occupancy Decode', icon: '🏗️' },
              { label: 'Reconstructed 3D', sub: 'Sparse Geometry', icon: '🎯' },
              { label: 'Classification', sub: 'ModelNet40 Accuracy', icon: '📊' }
            ].map((step, i, arr) => (
              <React.Fragment key={i}>
                <div style={{
                  ...
                  <div style={{ fontSize: '1.25rem', marginBottom: '2px' }}>{step.icon}</div>
          ...
          <div style={{ 
            ...
            <div>
              <span style={{ color: 'var(--text-dim)', display: 'block', fontSize: '0.75rem' }}>Sequence Length</span>
            ...
            <div>
              <span style={{ color: 'var(--text-dim)', display: 'block', fontSize: '0.75rem' }}>Target Dataset</span>
              <strong style={{ color: '#fff', fontFamily: 'var(--font-mono)' }}>{techSpecs.numClasses} ModelNet40 Classes</strong>
            </div>
            <div>
              <span style={{ color: 'var(--text-dim)', display: 'block', fontSize: '0.75rem' }}>Extended Vocabulary</span>
              <strong style={{ color: 'var(--accent-emerald)', fontFamily: 'var(--font-mono)' }}>297 Tokens (BOS=256, Class=257-296)</strong>
            </div>
          </div>

        </div>

        {/* 3. Previous Baseline Findings Context */}
        <div className="glass-card" style={{ marginBottom: '3rem', padding: '1.5rem 1.75rem' }}>
          <h3 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-cyan)' }}>
            <Activity size={18} /> Baseline 3D Pipeline Findings (Rudhra Joshi)
          </h3>
        ...
        {/* 4. Three Research Questions Tabs */}
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          {researchQuestions.map((rq) => {
            ...
            return (
              <div key={rq.id} className="glass-card" style={{ borderLeft: '4px solid var(--accent-cyan)', padding: '2rem' }}>
                ...
                <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#fff', marginBottom: '1rem', lineHeight: 1.3 }}>
                  {rq.question}
                </h3>
                ...
                {/* RQ3 Specific Content (Centerpiece) */}
                {rq.id === 'rq3' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div>
                      <h4 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#fff', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Bug size={18} color="var(--accent-amber)" /> Experimental Debugging & Research Engineering Contributions
                      </h4>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
                        {rq.debugging.map((bug, i) => (
                          <div key={i} style={{ background: 'rgba(9, 13, 22, 0.8)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: 'var(--radius-sm)', padding: '1.25rem' }}>
                            <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--accent-cyan)', marginBottom: '6px' }}>
                              {bug.title}
                            </div>
                        ...
                        <div style={{ minWidth: '550px' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', flexWrap: 'wrap', marginTop: '1rem', fontSize: '0.8rem' }}>
                            <span style={{ color: '#38bdf8', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                              <span style={{ width: '12px', height: '3px', background: '#38bdf8', borderRadius: '2px' }} /> NanoGPT Conditional
                            </span>
                            <span style={{ color: '#10b981', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                              <span style={{ width: '12px', height: '3px', background: '#10b981', borderRadius: '2px' }} /> MONAI Conditional
                            </span>
                            <span style={{ color: '#a855f7', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '6px' }}>
                              <span style={{ width: '12px', height: '3px', background: '#a855f7', borderRadius: '2px' }} /> NanoGPT Unconditional
                            </span>
                            <span style={{ color: '#f59e0b', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '6px' }}>
                              <span style={{ width: '12px', height: '3px', background: '#f59e0b', borderRadius: '2px' }} /> MONAI Unconditional
                            </span>
                          </div>
                        </div>
                      </div>
        ...
        <div id="my-contribution" className="glass-card" style={{ marginBottom: '3rem', padding: '2rem', borderLeft: '4px solid var(--accent-emerald)' }}>
          <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#fff', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <CheckCircle2 size={22} color="var(--accent-emerald)" /> My Specific Contributions
          </h3>
        ...
        {/* 6. Research Skills Demonstrated */}
        <div className="glass-card" style={{ padding: '1.75rem' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff', marginBottom: '1rem' }}>
            Research Capabilities & Technical Competencies Demonstrated
          </h3>

          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {demonstratedSkills.map((skill, i) => (
              <span key={i} style={{ fontSize: '0.825rem', padding: '6px 14px', borderRadius: 'var(--radius-full)', background: 'rgba(255, 255, 255, 0.04)', border: '1px solid rgba(255, 255, 255, 0.08)', color: 'var(--text-muted)' }}>
                {skill}
              </span>
            ))}
          </div>
